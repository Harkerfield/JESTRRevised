...
const MapComponent = ({ points }) => {
  ...
  useEffect(() => {
    ...
    if (points.length !== 0) {
      map.getView().fit(vectorSource.getExtent(), { padding: [30, 30, 30, 30], constrainResolution: false });
    } else {
      map.getView().centerOn(fromLonLat([-146.44166473513687, 64.31714411488758]), size, [570, 500])
    }
  ...
  return (
    <>
      <div id="map" className="map" style={{ height: '80vh', minWidth: '50%' }}></div>
    </>
  );
};
